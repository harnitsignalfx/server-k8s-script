...
def launchPods(namespace):
    ...
    namespaceExists = subprocess.Popen(["/bin/bash","-c","/k8s-demo-deploy/k8s-scripts/namespaceExists.sh",namespace])
    ...
    p = subprocess.Popen(["/bin/bash","-c","/k8s-demo-deploy/k8s-scripts/setup.sh",namespace])
...
def cleanPods(namespace):
    ...
    print("Executing setup.sh for ",namespace)
    p = subprocess.Popen(["/bin/bash","-c","/k8s-demo-deploy/k8s-scripts/cleanup.sh",namespace])
...
def deployWorkload(namespace):
    ...
    p = subprocess.Popen(["/bin/bash","-c","/k8s-demo-deploy/k8s-scripts/createBrokenStuff.sh",namespace])
